...
# Connect to an exist cluster etcd
#RUN trap "etcdctl rmdir $LOCALIP" EXIT SIGTERM SIGHUP 

# Ports to be exposed from the container for JMeter Slaves/Server
EXPOSE 1099 50000 2379 2380

# Copy the script file for the entrypoint
COPY launch_etcd_and_jmeter.pl /run/

# Run jmeter slave
ENTRYPOINT ["/run/launch_etcd_and_jmeter.pl"]
